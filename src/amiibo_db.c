 ...
 * @details Builds and validates the compact on-device index, streams JSON
 * sources, and serves category, figure, search, and game queries.
 */
...
#define AZ_SORT_BATCH_SCAN_RECORDS 16U
...
typedef struct {
  ...
  uint32_t
      reserved; /**< Reserved field retained for binary-format stability. */
} AzSourceStamp;

/** @brief Binary index header describing format, source fingerprints, counts,
 * and section offsets. */
typedef struct {
  ...
  uint16_t
      category_record_size;    /**< Serialized category-record size in bytes. */
  uint16_t figure_record_size; /**< Serialized figure-record size in bytes. */
  uint16_t
      game_record_size; /**< Serialized game-reference-record size in bytes. */
...
/** @brief Reference to a game JSON object keyed by an Amiibo identifier
 * pattern. */
typedef struct {
  uint8_t pattern[8];   /**< Eight-byte identifier pattern used to match Amiibo
                           figures. */
...
typedef struct {
  ...
  uint32_t current_offset; /**< Current absolute source offset reported by the
                              scanner. */
} AzScanControl;

/** @brief State used to map byte progress into a database progress range. */
typedef struct {
  AzDbProgressCallback
      callback;            /**< Progress callback supplied by the caller. */
...
typedef struct {
  ...
  uint8_t unicode_left; /**< Number of hexadecimal digits still expected for a
                           Unicode escape. */
...
typedef struct {
  ...
  bool
      in_figure; /**< Whether parser state is inside an Amiibo figure object. */
  uint32_t object_start;   /**< Source offset where the current figure object
                              begins. */
...
/** @brief Streaming-parser state used while constructing game-reference
 * records. */
typedef struct {
  AzScanControl control; /**< Shared streaming-scan control state. */
  ...
  uint32_t
      object_start; /**< Source offset where the current game object begins. */
...
/** @brief Streaming-parser state used to materialize game compatibility records
 * from a JSON range. */
typedef struct {
  AzScanControl control; /**< Shared streaming-scan control state. */
  ...
  bool saw_usage; /**< Whether the current game contained an explicit usage
                     section. */
  AzStringDecoder decoder; /**< Incremental JSON string decoder state. */
} AzGamesRangeContext;
...
/**
 * @brief Test whether one ASCII string contains another without case
 * sensitivity.
...
static bool az_contains_nocase(const char *haystack, const char *needle) {
  ...
  for (const char *h = haystack; *h; ++h) {
    size_t i = 0;
    while (i < nlen && h[i] &&
           az_ascii_lower(h[i]) == az_ascii_lower(needle[i]))
      i++;
    if (i == nlen)
      return true;
  }
 ...
 * @param c ASCII character or byte to inspect.
 * @return Nibble value from 0 through 15, or -1 for a non-hexadecimal
 * character.
 */
...
/**
 * @brief Parse a textual Amiibo identifier into binary and canonical
 * hexadecimal forms.
 * @param text Text to process or display.
 * @param out Destination for the computed result.
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_parse_id_text(const char *text, uint8_t out[8],
                             char out_hex[17]) {
  if (!text)
    return false;
  if (text[0] == '0' && (text[1] == 'x' || text[1] == 'X'))
 ...
 */
static const char *az_lw_key(const lwjson_stream_parser_t *parser,
                             size_t ordinal) {
  if (!parser)
    return NULL;
  for (size_t i = parser->stack_pos; i > 0; i--) {
    const lwjson_stream_stack_t *item = &parser->stack[i - 1];
    if (item->type != LWJSON_STREAM_TYPE_KEY)
      continue;
    if (ordinal == 0)
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_lw_key_is(const lwjson_stream_parser_t *parser, size_t ordinal,
                         const char *key) {
...
/**
 * @brief Determine the source offset of the current streamed JSON string
 * fragment.
 * @param parser Streaming JSON parser state.
 * @return The computed result value.
 */
 ...
 */
static void az_text_append(char *destination, size_t capacity, size_t *position,
                           char value) {
  if (!destination || !position || capacity == 0 || *position + 1 >= capacity)
    return;
 ...
 */
static void az_text_append_codepoint(char *destination, size_t capacity,
                                     size_t *position, uint16_t codepoint) {
  if (codepoint < 0x80) {
 ...
 */
static void az_copy_stream_string(const lwjson_stream_parser_t *parser,
                                  char *destination, size_t capacity,
                                  AzStringDecoder *decoder) {
  if (!parser || !destination || !decoder || capacity == 0)
    return;
  ...
  for (size_t i = 0; i < parser->data.str.buff_pos; i++) {
    ...
    if (decoder->unicode_left) {
      ...
      if (decoder->unicode_left == 0)
        az_text_append_codepoint(destination, capacity, &output_pos,
                                 decoder->unicode);
      continue;
    }
    if (decoder->escaped) {
      ...
      case '\\':
      case '/':
        az_text_append(destination, capacity, &output_pos, c);
        break;
      case 'b':
      case 'f':
      case 'n':
 ...
 */
static void az_progress_emit(AzDbProgressCallback callback, void *context,
                             AzDbProgressStage stage, uint8_t percent) {
  if (percent > 100U)
    percent = 100U;
  if (callback)
    callback(context, stage, percent);
}
 ...
 * @param context Caller-owned callback context.
 * @param stage Database progress phase.
 * @param start_percent Progress percentage assigned to the start of the
 * operation.
 ...
 */
static void az_progress_reporter_init(AzProgressReporter *reporter,
                                      AzDbProgressCallback callback,
                                      void *context, AzDbProgressStage stage,
 ...
 */
static void az_progress_report_bytes(AzProgressReporter *reporter,
                                     uint32_t consumed) {
  if (!reporter || !reporter->callback)
    return;
  ...
  if (percent != reporter->last_percent) {
    ...
    az_progress_emit(reporter->callback, reporter->context, reporter->stage,
                     percent);
  }
}
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_scan_json_file(Storage *storage, const char *path,
                              lwjson_stream_parser_callback_fn callback,
                              void *callback_context, AzScanControl *control,
                              AzProgressReporter *progress,
                              uint64_t *source_size) {
  if (!storage || !path || !callback || !control)
    return false;
  control->stop = false;
  ...
  if (ok && done)
    az_progress_report_bytes(progress,
                             progress ? progress->total_bytes : absolute);
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_scan_json_range(Storage *storage, const char *path,
                               uint32_t offset, uint32_t length,
                               lwjson_stream_parser_callback_fn callback,
                               void *callback_context, AzScanControl *control) {
  if (!storage || !path || !callback || !control || length == 0)
    return false;
  control->stop = false;
  control->failed = false;
  ...
  lwjson_stream_parser_t *parser = ok ? malloc(sizeof(*parser)) : NULL;
  if (!parser)
    ok = false;
  if (ok) {
    ok = lwjson_stream_init(parser, callback) == lwjsonOK &&
 ...
 */
static AzCategory *az_find_category(AzCategory *categories, uint16_t count,
                                    uint8_t id) {
...
/**
 * @brief Add a placeholder category when input references an unknown category
 * ID.
 * @param categories Category record array.
 * @param capacity Maximum number of elements or bytes available.
 * @param count Number of records or elements.
 ...
 * @return Pointer to the requested object, or NULL when it is not available.
 */
static AzCategory *az_add_unknown_category(AzCategory *categories,
                                           uint16_t capacity, uint16_t *count,
                                           uint8_t id) {
  if (!categories || !count || *count >= capacity)
    return NULL;
 ...
 * @return A negative, zero, or positive value when the left operand sorts
 * before, equal to, or after the right operand.
 */
static int az_category_compare(const AzCategory *left,
                               const AzCategory *right) {
 ...
 * @return true when the tested condition is satisfied; false otherwise.
 */
static bool az_lw_is_amiibo_entry(const lwjson_stream_parser_t *parser,
                                  uint8_t id[8], char id_hex[17]) {
  const char *entry = az_lw_key(parser, 0);
  if (!entry || !az_lw_key_is(parser, 1, "amiibos"))
    return false;
 ...
 */
static void az_figure_build_event(lwjson_stream_parser_t *parser,
  ...
  if (type == LWJSON_STREAM_TYPE_STRING &&
      az_lw_key_is(parser, 1, "amiibo_series")) {
    ...
    if (id_text && az_parse_hex_byte(id_text, &id)) {
      ...
      if (!category) {
        category = az_add_unknown_category(context->categories,
                                           context->category_capacity,
                                           context->category_count, id);
      }
      if (!category) {
      ...
      az_copy_stream_string(parser, category->name, sizeof(category->name),
                            &context->decoder);
  ...
  if (type == LWJSON_STREAM_TYPE_OBJECT) {
    ...
    if (az_lw_is_amiibo_entry(parser, id, hex)) {
      ...
      az_str_copy(context->current.id_hex, sizeof(context->current.id_hex),
                  hex);
  ...
  if (type == LWJSON_STREAM_TYPE_STRING && context->in_figure) {
    if (az_lw_key_is(parser, 0, "name") &&
      ...
      az_copy_stream_string(parser, context->current.name,
                            sizeof(context->current.name), &context->decoder);
      az_clean_field(context->current.name);
    } else if (az_lw_key_is(parser, 0, "na") &&
               az_lw_key_is(parser, 1, "release")) {
      az_copy_stream_string(parser, context->current.release_na,
  ...
  if (type == LWJSON_STREAM_TYPE_OBJECT_END && context->in_figure) {
    uint8_t id[8];
    if (!az_lw_is_amiibo_entry(parser, id, NULL))
      return;
    context->in_figure = false;
    context->current.json_length =
        context->control.current_offset - context->object_start + 1U;
    if (!context->current.name[0])
      az_str_copy(context->current.name, sizeof(context->current.name),
                  context->current.id_hex);
    AzCategory *category =
        az_find_category(context->categories, *context->category_count,
                         context->current.category);
    if (!category) {
      category = az_add_unknown_category(
          context->categories, context->category_capacity,
          context->category_count, context->current.category);
    }
    if (!category) {
      context->control.failed = true;
      context->control.stop = true;
    ...
    if (category->count < UINT16_MAX)
      category->count++;
    AzIndexFigureRecord record = {.figure = context->current};
 ...
 * @param type Type or event code to interpret.
 */
static void az_game_index_event(lwjson_stream_parser_t *parser,
                                lwjson_stream_type_t type) {
  ...
  if (type == LWJSON_STREAM_TYPE_OBJECT) {
    const char *entry = az_lw_key(parser, 0);
    if (entry && az_lw_key_is(parser, 1, "amiibos") &&
        az_parse_id_text(entry, context->pattern, NULL)) {
  ...
  if (type == LWJSON_STREAM_TYPE_OBJECT_END && context->in_entry) {
    ...
    const char *entry = az_lw_key(parser, 0);
    if (!entry || !az_lw_key_is(parser, 1, "amiibos") ||
        !az_parse_id_text(entry, pattern, NULL))
      return;
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_source_size(Storage *storage, const char *path,
                           uint64_t *out_size) {
  if (out_size)
    *out_size = 0;
  if (!storage || !path || !out_size)
    return false;
  FileInfo info;
  memset(&info, 0, sizeof(info));
  if (storage_common_stat(storage, path, &info) != FSE_OK ||
      (info.flags & FSF_DIRECTORY) || info.size > UINT32_MAX) {
    return false;
  }
  *out_size = info.size;
  return true;
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_source_stamp(Storage *storage, const char *path,
                            AzSourceStamp *out) {
  if (!out)
    return false;
  memset(out, 0, sizeof(*out));
  if (!az_source_size(storage, path, &out->size))
    return false;
  if (out->size == 0U)
    return true;
  ...
  for (size_t i = 0; i < COUNT_OF(offsets); i++) {
    ...
    size_t length = remaining > AZ_SOURCE_SAMPLE_BYTES ? AZ_SOURCE_SAMPLE_BYTES
                                                       : (size_t)remaining;
    if (!storage_file_seek(file, offset, true) ||
        storage_file_read(file, buffer, length) != length) {
      ok = false;
      break;
    }
  ...
  if (ok)
    ok = az_source_size(storage, path, &size_after) && size_after == out->size;
  if (ok)
    out->sample_crc32 = crc32;
 ...
 * @return true when the tested condition is satisfied; false otherwise.
 */
static bool az_stamp_equal(const AzSourceStamp *left,
                           const AzSourceStamp *right) {
...
static bool az_header_shape_valid(const AzIndexHeader *header) {
  return header &&
         memcmp(header->magic, AZ_INDEX_MAGIC, sizeof(header->magic)) == 0 &&
         header->version == AZ_INDEX_VERSION &&
         header->header_size == sizeof(AzIndexHeader) &&
 ...
 * @return true when the tested condition is satisfied; false otherwise.
 */
static bool az_header_layout_valid(const AzIndexHeader *header,
                                   uint64_t file_size) {
  if (!az_header_shape_valid(header) ||
      header->category_count > AZ_MAX_CATEGORIES)
    return false;
  uint64_t categories_offset = sizeof(AzIndexHeader);
  uint64_t figures_offset =
      categories_offset +
      (uint64_t)header->category_count * sizeof(AzIndexCategoryRecord);
  uint64_t games_offset = figures_offset + (uint64_t)header->figure_count *
                                               sizeof(AzIndexFigureRecord);
  uint64_t end_offset =
      games_offset + (uint64_t)header->game_ref_count * sizeof(AzIndexGameRef);
  return categories_offset == header->categories_offset &&
         figures_offset == header->figures_offset &&
         games_offset == header->games_offset && end_offset <= UINT32_MAX &&
 ...
 * @return true when the tested condition is satisfied; false otherwise.
 */
static bool az_json_range_valid(uint64_t source_size, uint32_t offset,
                                uint32_t length) {
...
static bool az_read_index_header(Storage *storage, AzIndexHeader *header) {
  if (!storage || !header || !storage_file_exists(storage, AZ_INDEX_FILE))
    return false;
  File *file = storage_file_alloc(storage);
  if (!file ||
      !storage_file_open(file, AZ_INDEX_FILE, FSAM_READ, FSOM_OPEN_EXISTING)) {
    if (file)
      storage_file_free(file);
    return false;
  }
...
/**
 * @brief Verify that an existing index is structurally valid and matches its
 * sources.
 * @param storage Storage service used for file operations.
 * @param out_header Destination for the resulting header.
 * @param progress_callback Optional callback that receives database progress
 * updates.
 * @param progress_context Caller-owned context passed to the progress callback.
 * @return true when the tested condition is satisfied; false otherwise.
 */
static bool az_index_current(Storage *storage, AzIndexHeader *out_header,
                             AzDbProgressCallback progress_callback,
                             void *progress_context) {
  AzIndexHeader header;
  if (!az_read_index_header(storage, &header))
    return false;

  uint64_t amiibo_size = 0;
  if (!az_source_size(storage, AZ_AMIIBO_JSON, &amiibo_size))
    return false;
  uint64_t games_size = 0;
  bool has_games = az_source_size(storage, AZ_GAMES_JSON, &games_size);
  if (!has_games)
    games_size = 0;
  if (header.amiibo_source.size != amiibo_size ||
      header.games_source.size != games_size) {
    az_progress_emit(progress_callback, progress_context, AzDbProgressChecking,
                     5U);
    return false;
  }

  AzSourceStamp amiibo;
  if (!az_source_stamp(storage, AZ_AMIIBO_JSON, &amiibo))
    return false;
  AzSourceStamp games;
  memset(&games, 0, sizeof(games));
  if (has_games && !az_source_stamp(storage, AZ_GAMES_JSON, &games))
    return false;
  az_progress_emit(progress_callback, progress_context, AzDbProgressChecking,
                   5U);
 ...
 */
static void az_build_category_ranks(const AzCategory *categories,
                                    uint16_t count, uint8_t ranks[256]) {
  memset(ranks, 0xFF, 256U);
  for (uint16_t i = 0; i < count; i++)
    ranks[categories[i].id] = (uint8_t)i;
}

/**
 * @brief Swap two indexed figure records.
 * @param left Left-side input value.
 * @param right Right-side input value.
 */
static void az_swap_figure_records(AzIndexFigureRecord *left,
                                   AzIndexFigureRecord *right) {
...
/**
 * @brief Restore heap ordering beneath one figure-record node.
 * @param records Figure record array.
 * @param count Number of records or elements.
 * @param root Heap node index to restore.
 */
static void az_figure_heap_sift_down(AzIndexFigureRecord *records,
                                     uint32_t count, uint32_t root) {
  while (count > 1U && root <= (count - 2U) / 2U) {
    uint32_t child = root * 2U + 1U;
    if (child + 1U < count &&
        az_figure_compare(&records[child].figure, &records[child + 1U].figure) <
            0) {
      child++;
    }
    if (az_figure_compare(&records[root].figure, &records[child].figure) >= 0)
      break;
    az_swap_figure_records(&records[root], &records[child]);
    root = child;
  }
}

/**
 * @brief Sort an in-memory batch of figure records by figure ordering.
 * @param records Figure record array.
 * @param count Number of records or elements.
 */
static void az_sort_category_records(AzIndexFigureRecord *records,
                                     uint32_t count) {
  if (!records || count < 2U)
    return;
  for (uint32_t start = count / 2U; start > 0U; start--) {
    az_figure_heap_sift_down(records, count, start - 1U);
  }
  for (uint32_t end = count; end > 1U; end--) {
    az_swap_figure_records(&records[0], &records[end - 1U]);
    az_figure_heap_sift_down(records, end - 1U, 0U);
  }
}

/**
 * @brief Write a contiguous block of figure records to an index file.
 * @param destination Destination object or buffer.
 * @param records Figure record array.
 * @param count Number of records or elements.
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_write_figure_record_block(File *destination,
                                         const AzIndexFigureRecord *records,
                                         uint32_t count) {
  if (!destination || (!records && count))
    return false;
  uint32_t written = 0;
  while (written < count) {
    uint32_t chunk = count - written;
    if (chunk > AZ_SORT_BATCH_WRITE_RECORDS)
      chunk = AZ_SORT_BATCH_WRITE_RECORDS;
    size_t bytes = (size_t)chunk * sizeof(AzIndexFigureRecord);
    if (storage_file_write(destination, records + written, bytes) != bytes)
      return false;
    written += chunk;
  }
  return true;
...
/**
 * @brief Sort figure records in bounded batches and write them to the
 * destination index.
 * @param storage Storage service used for file operations.
 * @param destination Destination object or buffer.
 * @param categories Category record array.
...
static bool az_write_sorted_figure_batches(
    Storage *storage, File *destination, const AzCategory *categories,
    uint16_t category_count, uint32_t figure_count, const uint8_t ranks[256],
    AzDbProgressCallback progress_callback, void *progress_context,
    bool *out_allocation_failed) {
  if (out_allocation_failed)
    *out_allocation_failed = false;
  if (!storage || !destination || !categories || !ranks)
    return false;
  az_progress_emit(progress_callback, progress_context, AzDbProgressSorting,
                   45U);
  if (figure_count == 0U) {
    az_progress_emit(progress_callback, progress_context, AzDbProgressSorting,
                     68U);
  ...
  for (uint16_t i = 0; i < category_count; i++) {
    if (categories[i].count > largest_category)
      largest_category = categories[i].count;
  }
  if (largest_category == 0U || largest_category > figure_count)
    return false;

  uint32_t capacity = AZ_SORT_BATCH_TARGET_RECORDS;
  if (capacity < largest_category)
    capacity = largest_category;
  if (capacity > figure_count)
    capacity = figure_count;

  AzIndexFigureRecord *batch =
      malloc((size_t)capacity * sizeof(AzIndexFigureRecord));
  if (!batch && capacity > largest_category) {
    capacity = largest_category;
    batch = malloc((size_t)capacity * sizeof(AzIndexFigureRecord));
  }
  if (!batch) {
    if (out_allocation_failed)
  ...
  bool ok = source && storage_file_open(source, AZ_INDEX_RAW, FSAM_READ,
                                        FSOM_OPEN_EXISTING);
  ...
  while (ok && batch_start < category_count) {
    ...
    while (batch_end < category_count) {
      ...
      if (batch_count > 0U && batch_count + category_records > capacity)
        break;
    ...
    while (ok && scanned < figure_count) {
      ...
      if (chunk > AZ_SORT_BATCH_SCAN_RECORDS)
        chunk = AZ_SORT_BATCH_SCAN_RECORDS;
      size_t bytes = (size_t)chunk * sizeof(AzIndexFigureRecord);
      ...
      for (uint32_t i = 0; i < chunk; i++) {
        ...
        batch[category_base[rank] + filled] = scan[i];
    ...
    for (uint16_t rank = batch_start; ok && rank < batch_end; rank++) {
      ...
      az_sort_category_records(batch + category_base[rank],
                               categories[rank].count);
    }
    if (ok)
      ok = az_write_figure_record_block(destination, batch, batch_count);
    if (!ok)
      break;
...
/**
 * @brief Compare indexed figure records using category ranks and figure
 * ordering.
 * @param left Left comparison operand.
 * @param right Right comparison operand.
 * @param ranks Category sort-rank lookup table.
 ...
 * before, equal to, or after the right operand.
 */
static int az_index_figure_compare(const AzIndexFigureRecord *left,
 ...
 * @param ranks Category sort-rank lookup table.
 */
static void az_sort_figure_run(AzIndexFigureRecord *records, uint16_t count,
                               const uint8_t ranks[256]) {
  for (uint16_t i = 1; i < count; i++) {
    AzIndexFigureRecord current = records[i];
    uint16_t j = i;
    while (j > 0 &&
           az_index_figure_compare(&records[j - 1], &current, ranks) > 0) {
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_create_sorted_runs(Storage *storage, uint32_t figure_count,
                                  const uint8_t ranks[256],
                                  AzDbProgressCallback progress_callback,
                                  void *progress_context) {
  File *source = storage_file_alloc(storage);
  ...
  bool ok =
      ...
      storage_file_open(destination, AZ_INDEX_SORT_RUNS, FSAM_WRITE,
                        FSOM_CREATE_ALWAYS);
  ...
  while (ok && processed < figure_count) {
    ...
    uint16_t chunk =
        (uint16_t)(remaining > AZ_SORT_RUN_RECORDS ? AZ_SORT_RUN_RECORDS
                                                   : remaining);
    size_t bytes = (size_t)chunk * sizeof(AzIndexFigureRecord);
    ...
    uint8_t percent =
        figure_count
            ? (uint8_t)(45U + ((uint64_t)processed * 8U) / figure_count)
            : 53U;
    az_progress_emit(progress_callback, progress_context, AzDbProgressSorting,
                     percent);
  }
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_flush_figure_records(File *destination,
                                    AzIndexFigureRecord *output,
                                    uint8_t *output_count) {
  if (!destination || !output || !output_count)
    return false;
  if (*output_count == 0U)
    return true;
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_merge_figure_pass(Storage *storage, const char *source_path,
                                 const char *destination_path,
                                 uint32_t figure_count, uint32_t run_size,
                                 const uint8_t ranks[256]) {
  ...
  bool ok =
      ...
      storage_file_open(destination, destination_path, FSAM_WRITE,
                        FSOM_CREATE_ALWAYS);
  ...
  for (uint32_t base = 0; ok && base < figure_count; base += run_size * 2U) {
    ...
    if (left_count > run_size)
      left_count = run_size;
    uint32_t right_count = figure_count - base - left_count;
    if (right_count > run_size)
      right_count = run_size;

    uint64_t left_offset = (uint64_t)base * sizeof(AzIndexFigureRecord);
    uint64_t right_offset =
        (uint64_t)(base + left_count) * sizeof(AzIndexFigureRecord);
    ...
    bool have_left = left_count > 0U &&
                     az_read_exact(left, &left_record, sizeof(left_record));
    bool have_right = right_count > 0U &&
                      az_read_exact(right, &right_record, sizeof(right_record));
    if (left_count > 0U && !have_left)
      ok = false;
    if (right_count > 0U && !have_right)
      ok = false;
    if (!ok)
      break;
    if (have_left)
...
/**
 * @brief Produce a fully sorted temporary figure stream using in-memory or
 * external sorting.
 * @param storage Storage service used for file operations.
 * @param figure_count Number of figure entries.
 * @param ranks Category sort-rank lookup table.
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_prepare_sorted_figures(Storage *storage, uint32_t figure_count,
                                      const uint8_t ranks[256],
                                      AzDbProgressCallback progress_callback,
                                      void *progress_context,
                                      const char **out_sorted_path) {
  if (!storage || !ranks || !out_sorted_path)
    return false;
  *out_sorted_path = AZ_INDEX_SORT_RUNS;
  az_progress_emit(progress_callback, progress_context, AzDbProgressSorting,
                   45U);
  if (!az_create_sorted_runs(storage, figure_count, ranks, progress_callback,
                             progress_context)) {
    return false;
  }

  uint32_t pass_count = 0;
  for (uint32_t size = AZ_SORT_RUN_RECORDS;
       size < figure_count && size <= UINT32_MAX / 2U; size *= 2U) {
    pass_count++;
  }
  ...
  for (uint32_t pass = 0; pass < pass_count; pass++) {
    if (!az_merge_figure_pass(storage, source_path, destination_path,
                              figure_count, run_size, ranks)) {
      return false;
    }
    const char *swap = source_path;
    source_path = destination_path;
    destination_path = swap;
    if (run_size <= UINT32_MAX / 2U)
      run_size *= 2U;
    uint8_t percent =
        (uint8_t)(53U + ((uint64_t)(pass + 1U) * 15U) / pass_count);
    az_progress_emit(progress_callback, progress_context, AzDbProgressSorting,
                     percent);
  }
  if (pass_count == 0U) {
...
/**
 * @brief Copy a fixed byte count between storage files while reporting
 * progress.
 * @param storage Storage service used for file operations.
 * @param source_path Path of the source file.
 * @param destination Destination object or buffer.
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_copy_file_bytes(Storage *storage, const char *source_path,
                               File *destination, uint32_t byte_count,
                               AzDbProgressCallback progress_callback,
                               void *progress_context, uint8_t start_percent,
                               uint8_t end_percent) {
  if (!storage || !source_path || !destination)
    return false;
  File *source = storage_file_alloc(storage);
  if (!source ||
  ...
  while (ok && copied < byte_count) {
    ...
    if (read != chunk ||
        storage_file_write(destination, buffer, read) != read) {
      ok = false;
      break;
    }
...
static bool az_index_build(Storage *storage, uint32_t *out_count,
  ...
  if (!az_source_stamp(storage, AZ_AMIIBO_JSON, &amiibo_before))
    return false;
  ...
  if (!raw_figures || !storage_file_open(raw_figures, AZ_INDEX_RAW, FSAM_WRITE,
                                         FSOM_CREATE_ALWAYS)) {
    ok = false;
  }
  if (ok) {
    AzFigureBuildContext figure_context = {
        .raw_file = raw_figures,
        .categories = categories,
        .category_capacity = AZ_MAX_CATEGORIES,
        .category_count = &category_count,
    };
    AzProgressReporter amiibo_progress;
    az_progress_reporter_init(&amiibo_progress, progress_callback,
                              progress_context, AzDbProgressAmiibo, 5U, 45U,
                              (uint32_t)amiibo_before.size);
    ok = az_scan_json_file(storage, AZ_AMIIBO_JSON, az_figure_build_event,
                           &figure_context, &figure_context.control,
                           &amiibo_progress, NULL);
  ...
  if (ok) {
    ...
    ok = index && storage_file_open(index, AZ_INDEX_TMP, FSAM_READ_WRITE,
                                    FSOM_CREATE_ALWAYS);
  }
  if (ok) {
  ...
  if (ok) {
    ...
    ok = az_write_sorted_figure_batches(
        storage, index, categories, category_count, figure_count,
        category_ranks, progress_callback, progress_context,
        &batch_allocation_failed);
    if (!ok && batch_allocation_failed) {
      const char *sorted_figure_path = AZ_INDEX_SORT_RUNS;
      ok = az_prepare_sorted_figures(storage, figure_count, category_ranks,
      ...
      if (ok) {
        ...
        ok = figure_bytes64 <= UINT32_MAX &&
             az_copy_file_bytes(storage, sorted_figure_path, index,
                                (uint32_t)figure_bytes64, progress_callback,
                                progress_context, 68U, 75U);
      }
    } else if (ok) {
      az_progress_emit(progress_callback, progress_context, AzDbProgressSorting,
                       75U);
  ...
  if (ok && storage_file_tell(index) != header.games_offset)
    ok = false;

  if (ok && has_games) {
    AzGameIndexBuildContext game_context = {.output_file = index};
    AzProgressReporter games_progress;
    az_progress_reporter_init(&games_progress, progress_callback,
                              progress_context, AzDbProgressGames, 75U, 95U,
                              (uint32_t)games_before.size);
    ok = az_scan_json_file(storage, AZ_GAMES_JSON, az_game_index_event,
                           &game_context, &game_context.control,
                           &games_progress, NULL);
    game_ref_count = game_context.count;
  } else if (ok) {
    az_progress_emit(progress_callback, progress_context, AzDbProgressGames,
                     95U);
  }
  ...
  if (ok) {
    ...
    if (expected_end > UINT32_MAX || storage_file_tell(index) != expected_end) {
      ...
      header.games_source = games_before;
      ok = storage_file_seek(index, 0, true) &&
           az_write_exact(index, &header, sizeof(header));
    }
  }
  if (index) {
    if (ok)
  ...
  az_progress_emit(progress_callback, progress_context, AzDbProgressFinalizing,
                   96U);
  if (ok) {
    ...
    bool games_after_present =
        az_source_stamp(storage, AZ_GAMES_JSON, &games_after);
  ...
  if (ok) {
    ...
    if (had_old && storage_common_rename(storage, AZ_INDEX_FILE,
                                         AZ_INDEX_BACKUP) != FSE_OK) {
      ok = false;
    }
    if (ok &&
        storage_common_rename(storage, AZ_INDEX_TMP, AZ_INDEX_FILE) != FSE_OK) {
      ok = false;
      if (had_old)
        storage_common_rename(storage, AZ_INDEX_BACKUP, AZ_INDEX_FILE);
    } else if (ok && had_old) {
      storage_common_remove(storage, AZ_INDEX_BACKUP);
    }
  }
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_open_index(Storage *storage, File **out_file,
                          AzIndexHeader *out_header) {
  if (!storage || !out_file || !out_header)
    return false;
  *out_file = NULL;
  File *file = storage_file_alloc(storage);
  if (!file ||
  ...
  AzIndexHeader header;
  uint64_t file_size = storage_file_size(file);
  if (!az_read_exact(file, &header, sizeof(header)) ||
      !az_header_layout_valid(&header, file_size)) {
    storage_file_close(file);
    storage_file_free(file);
    return false;
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_read_category_record(File *file, const AzIndexHeader *header,
                                    uint16_t ordinal,
                                    AzIndexCategoryRecord *out) {
  if (!file || !header || !out || ordinal >= header->category_count)
    return false;
  uint32_t offset =
      header->categories_offset + (uint32_t)ordinal * sizeof(*out);
  return storage_file_seek(file, offset, true) &&
         az_read_exact(file, out, sizeof(*out));
}
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_read_figure_record(File *file, const AzIndexHeader *header,
                                  uint32_t ordinal, AzIndexFigureRecord *out) {
  if (!file || !header || !out || ordinal >= header->figure_count)
    return false;
  uint32_t offset = header->figures_offset + ordinal * sizeof(*out);
  if (!storage_file_seek(file, offset, true) ||
      !az_read_exact(file, out, sizeof(*out)))
    return false;
  return az_json_range_valid(header->amiibo_source.size,
                             out->figure.json_offset, out->figure.json_length);
}
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_find_category_record(File *file, const AzIndexHeader *header,
                                    uint8_t category,
                                    AzIndexCategoryRecord *out) {
  for (uint16_t i = 0; i < header->category_count; i++) {
    AzIndexCategoryRecord record;
    if (!az_read_category_record(file, header, i, &record))
      return false;
...
/**
 * @brief Calculate the first row of a fixed-height list window around a
 * selection.
...
static uint16_t az_window_start(uint16_t selection, uint16_t total) {
  ...
  if (start + AZ_LIST_ROWS > total)
    start = (uint16_t)(total - AZ_LIST_ROWS);
  return start;
}

/**
 * @brief Ensure the binary metadata index exists and matches the current JSON
 * sources.
 * @param storage Storage service used for file operations.
...
bool az_db_ensure_index(Storage *storage, bool force, uint32_t *out_count,
  ...
  az_progress_emit(progress_callback, progress_context, AzDbProgressChecking,
                   0U);
  if (!storage || !storage_file_exists(storage, AZ_AMIIBO_JSON))
    return false;
  AzIndexHeader header;
  if (!force &&
      az_index_current(storage, &header, progress_callback, progress_context)) {
    if (out_count)
      *out_count = header.figure_count;
    az_progress_emit(progress_callback, progress_context, AzDbProgressDone,
                     100U);
    return true;
  }
  bool ok =
      az_index_build(storage, out_count, progress_callback, progress_context);
  if (ok)
    az_progress_emit(progress_callback, progress_context, AzDbProgressDone,
                     100U);
  return ok;
 ...
 * @param out_window_start Destination for the absolute ordinal of the first
 * returned row.
 * @param out_total Destination for the total number of matching records.
 * @return true on success; false if the operation cannot be completed.
 */
bool az_db_get_category_window(Storage *storage, uint16_t selection,
  ...
  File *file;
  AzIndexHeader header;
  if (!out_rows || !az_open_index(storage, &file, &header))
    return false;
  uint16_t total = header.category_count;
  if (total && selection >= total)
 ...
 * @return true on success; false if the operation cannot be completed.
 */
bool az_db_get_category(Storage *storage, uint16_t category_index,
                        AzCategory *out) {
  File *file;
  AzIndexHeader header;
  if (!out || !az_open_index(storage, &file, &header))
    return false;
  AzIndexCategoryRecord record;
 ...
 * @return true on success; false if the operation cannot be completed.
 */
bool az_db_get_figure_window(Storage *storage, uint8_t category,
                             uint16_t selection,
                             AzFigure out_rows[AZ_LIST_ROWS],
                             uint8_t *out_row_count, uint16_t *out_window_start,
                             uint16_t *out_total) {
  if (out_row_count)
    *out_row_count = 0;
  if (out_window_start)
  ...
  for (uint16_t i = start; ok && i < total && rows < AZ_LIST_ROWS; i++) {
    ...
    ok = az_read_figure_record(file, &header, category_record.first_figure + i,
                               &record);
    if (ok)
      out_rows[rows++] = record.figure;
  }
  storage_file_close(file);
  storage_file_free(file);
  if (out_row_count)
 ...
 * @return true on success; false if the operation cannot be completed.
 */
bool az_db_get_figure(Storage *storage, uint8_t category, uint16_t figure_index,
                      AzFigure *out) {
  File *file;
  AzIndexHeader header;
  if (!out || !az_open_index(storage, &file, &header))
  ...
  AzIndexCategoryRecord category_record;
  bool ok =
      az_find_category_record(file, &header, category, &category_record) &&
      figure_index < category_record.category.count;
  AzIndexFigureRecord record;
  if (ok)
    ok = az_read_figure_record(
        file, &header, category_record.first_figure + figure_index, &record);
  if (ok)
    *out = record.figure;
  storage_file_close(file);
  storage_file_free(file);
...
static bool az_figure_matches_query(const AzFigure *figure, const char *query) {
  return figure && (az_contains_nocase(figure->name, query) ||
                    az_contains_nocase(figure->id_hex, query));
}
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_search_count(File *file, const AzIndexHeader *header,
                            const char *query, uint16_t *out_total) {
  uint32_t count = 0;
  for (uint32_t i = 0; i < header->figure_count; i++) {
    AzIndexFigureRecord record;
    if (!az_read_figure_record(file, header, i, &record))
      return false;
    if (az_figure_matches_query(&record.figure, query))
      count++;
  }
 ...
 * @return true on success; false if the operation cannot be completed.
 */
bool az_db_search_window(Storage *storage, const char *query,
                         uint16_t selection, AzFigure out_rows[AZ_LIST_ROWS],
                         uint8_t *out_row_count, uint16_t *out_window_start,
                         uint16_t *out_total) {
  if (out_row_count)
    *out_row_count = 0;
  ...
  uint16_t match = 0;
  uint8_t rows = 0;
  for (uint32_t i = 0; ok && i < header.figure_count && rows < AZ_LIST_ROWS;
       i++) {
    AzIndexFigureRecord record;
    ok = az_read_figure_record(file, &header, i, &record);
    if (!ok || !az_figure_matches_query(&record.figure, query ? query : ""))
      continue;
    if (match >= start)
      out_rows[rows++] = record.figure;
    match++;
 ...
 * @return true on success; false if the operation cannot be completed.
 */
bool az_db_search_get(Storage *storage, const char *query, uint16_t match_index,
                      AzFigure *out) {
  File *file;
  AzIndexHeader header;
  if (!out || !az_open_index(storage, &file, &header))
  ...
  for (uint32_t i = 0; i < header.figure_count; i++) {
    ...
    if (!az_read_figure_record(file, &header, i, &record))
      break;
    if (!az_figure_matches_query(&record.figure, query ? query : ""))
      continue;
...
bool az_db_find_by_id(Storage *storage, const uint8_t id[8], AzFigure *out) {
  File *file;
  AzIndexHeader header;
  if (!id || !out || !az_open_index(storage, &file, &header))
    return false;
  bool found = false;
  for (uint32_t i = 0; i < header.figure_count; i++) {
    AzIndexFigureRecord record;
...
static const char *az_platform_label(const char *key) {
  if (!key)
    return NULL;
  if (strcmp(key, "games3DS") == 0)
    return "3DS";
  if (strcmp(key, "gamesWiiU") == 0)
    return "Wii U";
  if (strcmp(key, "gamesSwitch") == 0)
    return "Switch";
  if (strcmp(key, "gamesSwitch2") == 0)
    return "Switch 2";
  return NULL;
}
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_game_duplicate(const AzGame *games, uint16_t count,
                              const char *platform, const char *name,
                              const char *usage) {
  for (uint16_t i = 0; i < count; i++) {
    if (strcmp(games[i].platform, platform) == 0 &&
        strcmp(games[i].name, name) == 0 && strcmp(games[i].usage, usage) == 0)
      return true;
  }
  return false;
}
 ...
 */
static void az_game_add(AzGamesRangeContext *context, const char *usage_text,
                        bool writes) {
  if (!context || !context->game_name[0] ||
      context->count >= context->max_games)
    return;
  const char *text = usage_text && usage_text[0] ? usage_text : "Compatible";
  if (az_game_duplicate(context->games, context->count, context->platform,
                        context->game_name, text))
    return;
...
/**
 * @brief Consume one streaming JSON event while materializing game
 * compatibility data.
 * @param parser Streaming JSON parser state.
 * @param type Type or event code to interpret.
 */
static void az_games_range_event(lwjson_stream_parser_t *parser,
                                 lwjson_stream_type_t type) {
  ...
  if (type == LWJSON_STREAM_TYPE_STRING) {
    if (context->in_game && az_lw_key_is(parser, 0, "gameName")) {
      az_copy_stream_string(parser, context->game_name,
                            sizeof(context->game_name), &context->decoder);
      az_clean_field(context->game_name);
      if (parser->data.str.is_last)
        az_game_flush_pending(context);
    } else if (context->in_usage && az_lw_key_is(parser, 0, "Usage")) {
      az_copy_stream_string(parser, context->usage.text,
                            sizeof(context->usage.text), &context->decoder);
      az_clean_field(context->usage.text);
    }
    return;
  }
  if (type == LWJSON_STREAM_TYPE_TRUE && context->in_usage &&
      az_lw_key_is(parser, 0, "write")) {
    context->usage.writes = true;
    return;
  }
  if (type == LWJSON_STREAM_TYPE_FALSE && context->in_usage &&
      az_lw_key_is(parser, 0, "write")) {
  ...
  if (type == LWJSON_STREAM_TYPE_OBJECT_END) {
    ...
    if (context->in_game && platform && parser->stack_pos > 0 &&
        parser->stack[parser->stack_pos - 1].type == LWJSON_STREAM_TYPE_ARRAY) {
      az_game_flush_pending(context);
      if (!context->saw_usage)
        az_game_add(context, "Compatible", false);
...
/**
 * @brief Parse one indexed game JSON range and append matching compatibility
 * entries.
 * @param storage Storage service used for file operations.
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_parse_game_ref(Storage *storage, const AzIndexGameRef *ref,
                              AzGame *games, uint16_t max_games,
                              uint16_t *in_out_count) {
  AzGamesRangeContext *context = calloc(1, sizeof(*context));
  if (!context)
    return false;
  context->games = games;
  context->max_games = max_games;
  context->count = *in_out_count;
  bool ok = az_scan_json_range(storage, AZ_GAMES_JSON, ref->json_offset,
                               ref->json_length, az_games_range_event, context,
                               &context->control);
 ...
 * @return true on success; false if the operation cannot be completed.
 */
bool az_db_load_games(Storage *storage, const uint8_t id[8], AzGame *out_games,
                      uint16_t max_games, uint16_t *out_count) {
  if (out_count)
    *out_count = 0;
  if (!storage || !id || !out_games || !out_count || max_games == 0)
    return false;
  File *file;
  AzIndexHeader header;
  if (!az_open_index(storage, &file, &header))
    return false;
  ...
  uint16_t count = 0;
  for (uint32_t i = 0; ok && i < header.game_ref_count && count < max_games;
       i++) {
    AzIndexGameRef ref;
    ok = az_read_exact(file, &ref, sizeof(ref));
    if (!ok)
      break;
    if (!az_json_range_valid(header.games_source.size, ref.json_offset,
                             ref.json_length)) {
      ok = false;
      break;
    }
