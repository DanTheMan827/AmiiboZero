 ...
 * @details Scans saved tags and Lock-On payloads, validates companion data, and
 * manages file naming, rename, and deletion operations.
 */
 ...
 */
static bool az_saved_full_path(const char *filename, char *out,
                               size_t out_size);
...
/**
 * @brief Assemble a saved-figure path from a sanitized name, identifier, and
 * suffix.
 * @param out Destination for the computed result.
 * @param out_size Destination for the resulting size.
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_build_save_path(char *out, size_t out_size,
                               const char *safe_name, const char *id_hex,
                               const char *suffix) {
  if (!out || out_size == 0)
    return false;
  out[0] = '\0';
  return az_append(out, out_size, AZ_FIGURES_DIR) &&
         az_append(out, out_size, "/") && az_append(out, out_size, safe_name) &&
         az_append(out, out_size, "_") && az_append(out, out_size, id_hex) &&
         az_append(out, out_size, suffix);
}
...
static uint16_t az_lockon_crc16(const uint8_t *data, size_t length) {
  ...
  for (size_t i = 0; i < length; i++) {
    ...
    for (uint8_t bit = 0; bit < 8; bit++) {
      crc =
          (crc & 1U) ? (uint16_t)((crc >> 1) ^ 0x8408U) : (uint16_t)(crc >> 1);
    }
  }
  return crc;
}
...
/**
 * @brief Check whether a Lock-On file size is within the supported payload
 * range.
 * @param size Number of bytes in the supplied buffer or file.
 * @return true when the tested condition is satisfied; false otherwise.
 */
static bool az_lockon_size_valid(uint64_t size) {
  return (size >= 1U && size <= AZ_LOCKON_PAYLOAD_MAX) ||
         size == AZ_LOCKON_SRAM_SIZE;
}
...
uint16_t az_saved_scan(Storage *storage, AzSavedEntry *out,
  ...
  while (count < max_entries &&
    ...
    if ((info.flags & FSF_DIRECTORY) || !az_has_nfc_extension(name))
      continue;
    ...
    entry->valid = az_nfc_load_device(device, path) &&
                   az_nfc_extract_figure_id(device, entry->id);
    if (!entry->valid)
      continue;
...
uint16_t az_lockon_scan(Storage *storage, AzLockOnEntry *out,
  ...
  while (count < max_entries &&
         storage_dir_read(dir, &info, name, sizeof(name))) {
    if ((info.flags & FSF_DIRECTORY) || !az_lockon_size_valid(info.size))
      continue;
...
bool az_lockon_load(Storage *storage, const char *filename,
  ...
  if (!storage || !filename || !out_sram || strchr(filename, '/') ||
      strchr(filename, '\\')) {
    return false;
  }
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_saved_lockon_path(const char *saved_filename, char *out,
                                 size_t out_size) {
  if (!saved_filename || !out || out_size == 0)
    return false;
  char nfc_path[AZ_PATH_MAX];
  if (!az_saved_full_path(saved_filename, nfc_path, sizeof(nfc_path)))
    return false;
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_saved_lockon_work_path(const char *saved_filename,
                                      const char *suffix, char *out,
                                      size_t out_size) {
  if (!saved_filename || !suffix || !out || out_size == 0)
    return false;
  char base[AZ_PATH_MAX];
  if (!az_saved_lockon_path(saved_filename, base, sizeof(base)))
    return false;
...
bool az_saved_lockon_load(Storage *storage, const char *saved_filename,
                          uint8_t out_sram[AZ_LOCKON_SRAM_SIZE]) {
  if (!storage || !saved_filename || !out_sram)
    return false;
  char path[AZ_PATH_MAX];
  ...
  if (storage_file_open(file, path, FSAM_READ, FSOM_OPEN_EXISTING) &&
      ...
      storage_file_read(file, out_sram, AZ_LOCKON_SRAM_SIZE) ==
          AZ_LOCKON_SRAM_SIZE) {
    uint16_t crc = az_lockon_crc16(out_sram, AZ_LOCKON_PAYLOAD_MAX);
...
bool az_saved_lockon_save(Storage *storage, const char *saved_filename,
  ...
  if (!storage || !saved_filename || !sram)
    return false;
  ...
  if (!az_saved_lockon_path(saved_filename, path, sizeof(path)) ||
      !az_saved_lockon_work_path(saved_filename, ".tmp", tmp_path,
                                 sizeof(tmp_path)) ||
      !az_saved_lockon_work_path(saved_filename, ".bak", backup_path,
                                 sizeof(backup_path))) {
    return false;
  }

  if (storage_common_exists(storage, tmp_path))
  ...
  if (storage_file_open(file, tmp_path, FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
    written = storage_file_write(file, sram, AZ_LOCKON_SRAM_SIZE) ==
                  AZ_LOCKON_SRAM_SIZE &&
  ...
  if (storage_common_exists(storage, backup_path))
...
/**
 * @brief Convert arbitrary display text into a filesystem-safe save-name
 * component.
...
void az_make_unique_save_path(Storage *storage, const AzFigure *figure,
  ...
  if (!storage || !figure || !out || out_size == 0)
    return;
  char safe[48];
  az_sanitize_filename(figure->name, safe, sizeof(safe));

  if (!az_build_save_path(out, out_size, safe, figure->id_hex, ".nfc"))
    return;
  if (!storage_common_exists(storage, out))
    return;

  for (unsigned i = 2; i < 1000; i++) {
    char suffix[16];
    snprintf(suffix, sizeof(suffix), "-%u.nfc", i);
    if (!az_build_save_path(out, out_size, safe, figure->id_hex, suffix))
      return;
    if (!storage_common_exists(storage, out))
      return;
  }
...
static void az_sanitize_rename(const char *in, char *out, size_t out_size) {
  ...
  if (input_len >= 4 && strcasecmp(in + input_len - 4, ".nfc") == 0)
    input_len -= 4;
  size_t p = 0;
  bool last_sep = false;
...
static bool az_saved_full_path(const char *filename, char *out,
                               size_t out_size) {
  if (!filename || !out || out_size == 0 || strchr(filename, '/') ||
      strchr(filename, '\\'))
    return false;
...
bool az_saved_rename(Storage *storage, const char *old_filename,
                     ...
                     size_t out_size) {
  if (!storage || !old_filename || !requested_name || !out_filename ||
      out_size == 0)
    return false;
  if (strchr(old_filename, '/') || strchr(old_filename, '\\'))
    return false;

  char safe[72];
  az_sanitize_rename(requested_name, safe, sizeof(safe));
  char old_path[AZ_PATH_MAX];
  if (!az_saved_full_path(old_filename, old_path, sizeof(old_path)))
    return false;
  ...
  if (!az_saved_full_path(candidate, new_path, sizeof(new_path)))
    return false;

  if (strcmp(candidate, old_filename) != 0 &&
      !az_saved_name_available(storage, candidate)) {
    bool found = false;
    for (unsigned suffix = 2; suffix < 1000; suffix++) {
      written =
          snprintf(candidate, sizeof(candidate), "%s-%u.nfc", safe, suffix);
      if (written <= 0 || (size_t)written >= sizeof(candidate))
        return false;
      if (!az_saved_full_path(candidate, new_path, sizeof(new_path)))
        return false;
  ...
  bool has_lockon =
      az_saved_lockon_path(old_filename, old_lockon, sizeof(old_lockon)) &&
      storage_common_exists(storage, old_lockon);
  if (has_lockon &&
      !az_saved_lockon_path(candidate, new_lockon, sizeof(new_lockon)))
    return false;

  if (storage_common_rename(storage, old_path, new_path) != FSE_OK)
    return false;
  if (has_lockon &&
      storage_common_rename(storage, old_lockon, new_lockon) != FSE_OK) {
...
bool az_saved_delete(Storage *storage, const char *filename) {
  if (!storage || !filename || strchr(filename, '/') || strchr(filename, '\\'))
    return false;
  char path[AZ_PATH_MAX];
  if (!az_saved_full_path(filename, path, sizeof(path)))
    return false;

  if (storage_common_remove(storage, path) != FSE_OK)
    return false;
