 ...
 * @details Loads retail key material, derives per-tag keys, encrypts or
 * decrypts dumps, and generates compatible identities.
 */
...
typedef struct {
  uint8_t
      aes_key[16]; /**< AES-128 key used for dump encryption or decryption. */
 ...
 */
static bool az_hmac_sha256(const uint8_t *key, size_t key_len,
...
/**
 * @brief Validate the variable-length metadata encoded in an 80-byte retail
 * key.
 * @param key Cryptographic key bytes.
...
/**
 * @brief Derive per-tag AES, IV, and HMAC material from a retail key and tag
 * state.
 * @param key Cryptographic key bytes.
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_derive_key(const uint8_t key[80], const uint8_t raw_uid[9],
                          const uint8_t write_counter[2],
                          const uint8_t salt[32], AzDerivedKey *out) {
  if (!az_tag_key_valid(key) || !raw_uid || !write_counter || !salt || !out)
    return false;
  ...
  if (!az_hmac_sha256(key, 16, input, seed_len + 2, h0))
    return false;
  input[1] = 1;
  if (!az_hmac_sha256(key, 16, input, seed_len + 2, h1))
    return false;
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_aes_ctr(const uint8_t key[16], const uint8_t iv[16],
  ...
  int rc = mbedtls_aes_crypt_ctr(&aes, length, &nc_off, nonce_counter,
...
/**
 * @brief Generate a standard raw Amiibo UID with required manufacturer and
 * checksum bytes.
 * @param raw_uid Nine-byte raw Amiibo UID buffer.
 */
...
/**
 * @brief Generate a version-3 seven-byte UID with the required manufacturer
 * prefix.
...
/**
 * @brief Calculate the tag HMAC over the plaintext Amiibo fields covered by the
 * tag key.
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_calculate_tag_hmac(const uint8_t plain[AZ_DUMP_SIZE],
                                  const AzDerivedKey *tag_key,
                                  uint8_t out[32]) {
  uint8_t tag_input[52];
  memcpy(tag_input, plain, 8);
  memcpy(tag_input + 8, plain + 84, 44);
  return az_hmac_sha256(tag_key->hmac_key, 16, tag_input, sizeof(tag_input),
                        out);
}
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_calculate_data_hmac(const uint8_t plain[AZ_DUMP_SIZE],
...
/**
 * @brief Encrypt a plaintext Amiibo dump and populate its authentication
 * fields.
 * @param plain Plaintext Amiibo dump bytes.
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_encrypt_plain_dump(const uint8_t plain[AZ_DUMP_SIZE],
                                  const AzKeys *keys,
                                  uint8_t out_dump[AZ_DUMP_SIZE]) {
  if (!plain || !keys || !keys->valid || !out_dump)
    return false;

  AzDerivedKey tag_key;
  AzDerivedKey data_key;
  if (!az_derive_key(keys->static_key, plain, plain + 17, plain + 96, &tag_key))
    return false;
  if (!az_derive_key(keys->data_key, plain, plain + 17, plain + 96, &data_key))
    return false;

  memcpy(out_dump, plain, AZ_DUMP_SIZE);
  uint8_t tag_hmac[32];
  uint8_t data_hmac[32];
  if (!az_calculate_tag_hmac(plain, &tag_key, tag_hmac))
    return false;
  if (!az_calculate_data_hmac(plain, &data_key, tag_hmac, data_hmac))
    return false;
  ...
  if (!az_aes_ctr(data_key.aes_key, data_key.aes_iv, crypt_input, crypt_output,
                  sizeof(crypt_input))) {
    return false;
  }
...
bool az_keys_load(Storage *storage, AzKeys *keys) {
  ...
  if (!file)
    return false;
  bool ok =
      storage_file_open(file, AZ_KEYS_FILE, FSAM_READ, FSOM_OPEN_EXISTING);
  if (ok && storage_file_size(file) == 160) {
    ...
    if (ok) {
      ...
      keys->valid = az_tag_key_valid(keys->data_key) &&
                    az_tag_key_valid(keys->static_key);
      ok = keys->valid;
    }
  }
...
bool az_decrypt_dump(const uint8_t encrypted_dump[AZ_DUMP_SIZE],
  ...
  if (!encrypted_dump || !keys || !keys->valid || !out_plain)
    return false;

  AzDerivedKey tag_key;
  AzDerivedKey data_key;
  if (!az_derive_key(keys->static_key, encrypted_dump, encrypted_dump + 17,
                     encrypted_dump + 96, &tag_key)) {
    return false;
  }
  if (!az_derive_key(keys->data_key, encrypted_dump, encrypted_dump + 17,
                     encrypted_dump + 96, &data_key)) {
    return false;
  }
  ...
  if (!az_calculate_tag_hmac(out_plain, &tag_key, expected_tag))
    return false;
  if (memcmp(expected_tag, encrypted_dump + 52, sizeof(expected_tag)) != 0)
    return false;
  if (!az_calculate_data_hmac(out_plain, &data_key, expected_tag,
                              expected_data))
    return false;
  if (memcmp(expected_data, encrypted_dump + 128, sizeof(expected_data)) != 0)
    return false;
  return true;
}
...
bool az_rekey_dump_uid(const uint8_t encrypted_dump[AZ_DUMP_SIZE],
  ...
  if (!encrypted_dump || !keys || !out_dump || !out_raw_uid)
    return false;
  uint8_t plain[AZ_DUMP_SIZE];
  if (!az_decrypt_dump(encrypted_dump, keys, plain))
    return false;
...
bool az_rekey_v3_dump_uid(const uint8_t encrypted_dump[AZ_DUMP_SIZE],
                          const AzKeys *keys, uint8_t out_dump[AZ_DUMP_SIZE],
                          uint8_t out_uid7[7]) {
  if (!encrypted_dump || !keys || !out_dump || !out_uid7)
    return false;
  uint8_t plain[AZ_DUMP_SIZE];
  if (!az_decrypt_dump(encrypted_dump, keys, plain))
    return false;
...
bool az_generate_dump(const uint8_t figure_id[8], const AzKeys *keys,
  ...
  if (!figure_id || !keys || !keys->valid || !out_dump || !out_raw_uid)
    return false;
  ...
  static const uint8_t suffix[12] = {
      0x01, 0x00, 0x0F, 0xBD, 0x00, 0x00, 0x00, 0x04, 0x5F, 0x00, 0x00, 0x00,
  };
...
bool az_generate_v3_dump(const uint8_t figure_id[8], const AzKeys *keys,
  ...
  if (!figure_id || !keys || !keys->valid || !out_dump || !out_uid7)
    return false;
