 ...
 * @details Creates standard and version-3 tag images, implements listener
 * behavior, validates dumps, and extracts tag metadata.
 */
 ...
 */
bool az_figure_is_v3(const uint8_t id[8]) { return id && id[7] == 0x03; }
 ...
 */
static bool az_nfc_set_identity(MfUltralightData *data,
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_nfc_set_identity_v3(MfUltralightData *data,
                                   const uint8_t uid7[7]) {
  if (!data || !uid7 || !mf_ultralight_set_uid(data, uid7, 7U))
    return false;
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_nfc_install_standard(MfUltralightData *data,
                                    const uint8_t dump[AZ_DUMP_SIZE],
                                    const uint8_t raw_uid[9]) {
  static const uint8_t version[8] = {0x00, 0x04, 0x04, 0x02,
                                     0x01, 0x00, 0x11, 0x03};
  ...
  if (!az_nfc_set_identity(data, raw_uid))
    return false;
 ...
 * @param data Data buffer or tag state used by the operation.
 */
static void az_nfc_expand_v3(const uint8_t standard[AZ_NTAG215_BYTES],
                             MfUltralightData *data) {
  memset(data->page, 0, sizeof(data->page));
  uint8_t *sector0 = (uint8_t *)data->page;

  memcpy(sector0, standard, AZ_V3_SHIFT_START);
  memcpy(sector0 + AZ_V3_SHIFT_START + AZ_V3_SHIFT_SIZE,
         standard + AZ_V3_SHIFT_START, AZ_NTAG215_BYTES - AZ_V3_SHIFT_START);
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_nfc_install_v3(MfUltralightData *data,
                              const uint8_t dump[AZ_DUMP_SIZE],
                              const uint8_t uid7[7]) {
  static const uint8_t version[8] = {0x00, 0x04, 0x04, 0x05,
                                     0x02, 0x02, 0x15, 0x03};
  ...
  if (!az_nfc_set_identity_v3(data, uid7))
    return false;
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_nfc_extract_standard_dump(const MfUltralightData *data,
  ...
  if (data->type == MfUltralightTypeNTAGI2CPlus2K &&
      data->pages_total >= AZ_V3_PAGES) {
    memcpy(out_dump, bytes, AZ_V3_SHIFT_START);
    memcpy(out_dump + AZ_V3_SHIFT_START,
 ...
 * @return true on success; false if the operation cannot be completed.
 */
bool az_nfc_generate_device(NfcDevice *device, const AzFigure *figure,
                            const AzKeys *keys) {
  if (!device || !figure || !keys || !keys->valid)
    return false;
...
bool az_nfc_device_is_v3(const NfcDevice *device) {
  if (!az_nfc_validate_amiibo(device))
    return false;
  const MfUltralightData *data = (const MfUltralightData *)nfc_device_get_data(
      device, NfcProtocolMfUltralight);
...
/** @brief Internal outcome of a version-3 NFC command before listener
 * post-processing. */
typedef enum {
  AzV3CommandNotFound,     /**< Received command is not handled by the version-3
                              dispatcher. */
  ...
  AzV3CommandProcessedAck, /**< Command completed and requires an ACK response.
                            */
  AzV3CommandProcessedSilent, /**< Command completed without transmitting a
                                 response. */
  AzV3CommandNotProcessedNak, /**< Command was rejected with a generic NAK. */
  AzV3CommandNotProcessedAuthNak, /**< Command was rejected because
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_v3_send_standard(AmiiboZeroApp *app, const uint8_t *data,
                                size_t size) {
  if (!app || !app->v3_tx_buffer || !data || size > AZ_V3_MAX_RESPONSE_BYTES)
    return false;
...
static MfUltralightData *az_v3_data(AmiiboZeroApp *app) {
  if (!app || !app->nfc_device)
    return NULL;
  return (MfUltralightData *)nfc_device_get_data(app->nfc_device,
...
static bool az_v3_page_valid(const AmiiboZeroApp *app, uint16_t page) {
  ...
  case 2:
    return false;
  case 3:
    return page >= AZ_V3_MIRROR_SESSION_PAGE &&
           page <= AZ_V3_MIRROR_NS_REG_PAGE;
  default:
    return false;
  }
}
 ...
 * @return true when the tested condition is satisfied; false otherwise.
 */
static bool az_v3_range_valid(const AmiiboZeroApp *app, uint16_t start_page,
                              uint16_t end_page) {
  if (!app)
    return false;
  switch (app->v3_sector) {
  case 0:
    if (start_page <= 0xE9U && end_page <= 0xE9U)
      return true;
    if (start_page >= AZ_V3_SESSION_PAGE && end_page <= AZ_V3_NS_REG_PAGE)
      return true;
    return start_page >= AZ_V3_SRAM_FIRST_PAGE &&
           end_page <= AZ_V3_SRAM_LAST_PAGE;
  case 1:
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_v3_native_page(const AmiiboZeroApp *app, uint16_t page,
                              uint16_t *native_page) {
  if (!app || !native_page || !az_v3_page_valid(app, page))
    return false;
...
/**
 * @brief Determine whether a logical page targets the version-3 NS register
 * window.
 * @param app Application state.
 * @param page Logical tag page number.
 * @return true when the tested condition is satisfied; false otherwise.
 ...
 */
static void az_v3_read_page(AmiiboZeroApp *app, const MfUltralightData *data,
                            uint16_t page, uint8_t out[4]) {
  memset(out, 0, 4U);
  if (!app || !data || !az_v3_page_valid(app, page))
    return;

  if (app->v3_sector == 0 && page >= AZ_V3_SRAM_FIRST_PAGE &&
      page <= AZ_V3_SRAM_LAST_PAGE) {
    const size_t offset = (size_t)(page - AZ_V3_SRAM_FIRST_PAGE) * 4U;
    memcpy(out, app->current_lockon_sram + offset, 4U);
    return;
  }

  if (page == AZ_V3_PWD_PAGE || page == AZ_V3_PACK_PAGE)
    return;

  uint16_t native_page = 0;
  if (!az_v3_native_page(app, page, &native_page) ||
      native_page >= data->pages_total)
    return;
 ...
 * @return true when the tested condition is satisfied; false otherwise.
 */
static bool az_v3_check_access(const AmiiboZeroApp *app,
                               const MfUltralightData *data,
                               uint16_t start_page, bool write_op) {
  if (!app || !data)
    return false;
  const MfUltralightConfigPages *config =
      (const MfUltralightConfigPages *)&data->page[AZ_V3_CONFIG_PAGE];

  if (!app->v3_authenticated && config->auth0 <= start_page &&
      (config->access.prot || write_op)) {
    return false;
  }
  if (config->access.cfglck && write_op) {

    const uint16_t config_page_start = (uint16_t)(data->pages_total - 4U);
    if (start_page == config_page_start || start_page == config_page_start + 1U)
      return false;
  }
  return true;
}
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_v3_static_page_locked(const MfUltralightData *data,
                                     uint16_t page) {
  if (!data || page < 3U || page > 15U)
    return false;
  const uint16_t locks =
      (uint16_t)data->page[2].data[2] | ((uint16_t)data->page[2].data[3] << 8);
 ...
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_v3_dynamic_page_locked(const MfUltralightData *data,
                                      uint16_t page, uint8_t sector) {
  if (!data)
    return false;
  const uint16_t linear_page = (uint16_t)(page + (uint16_t)sector * 256U);
  if (linear_page < 16U || linear_page > 511U)
    return false;
  const uint8_t bit = (uint8_t)((linear_page - 16U) / 32U);
  const uint16_t locks =
      (uint16_t)data->page[AZ_V3_DYNAMIC_LOCK_PAGE].data[0] |
 ...
 */
static void az_v3_write_static_locks(MfUltralightData *data,
  ...
  if (current & (1U << 3))
    incoming &= (uint16_t)~(1U << 3);
  if (current & 0x03F0U)
    incoming &= (uint16_t)~0x03F0U;
  if (current & 0xFC00U)
    incoming &= (uint16_t)~0xFC00U;
  current |= incoming;
 ...
 * @param payload Four-byte page or command payload.
 */
static void az_v3_write_dynamic_locks(MfUltralightData *data,
                                      const uint8_t payload[4]) {
  uint32_t current =
      (uint32_t)data->page[AZ_V3_DYNAMIC_LOCK_PAGE].data[0] |
...
/**
 * @brief Apply a version-3 page write while enforcing lock and register
 * behavior.
 * @param app Application state.
 * @param data Data buffer or tag state used by the operation.
 * @param page Logical tag page number.
 ...
 */
static AzV3CommandResult az_v3_write_page(AmiiboZeroApp *app,
                                          MfUltralightData *data, uint16_t page,
                                          const uint8_t payload[4]) {
  if (!app || !data || !payload || !az_v3_page_valid(app, page))
    return AzV3CommandNotProcessedNak;
  if (!az_v3_check_access(app, data, page, true))
    return AzV3CommandNotProcessedNak;
...
/**
 * @brief Determine whether configured authentication blocks the current
 * operation.
 * @param data Data buffer or tag state used by the operation.
 * @param config Ultralight configuration-page state.
 * @param auth_success Whether authentication has succeeded for the current
 * interaction.
 * @return true on success; false if the operation cannot be completed.
 */
static bool az_v3_auth_locked(MfUltralightData *data,
                              const MfUltralightConfigPages *config,
                              bool auth_success) {
  if (!data || !config || config->access.authlim == 0U)
    return false;
  const uint32_t limit = 1UL << config->access.authlim;
  if (data->auth_attempts >= limit)
    return true;
  if (auth_success)
 ...
 * @return The computed result value.
 */
static AzV3CommandResult az_v3_dispatch_command(AmiiboZeroApp *app,
                                                MfUltralightData *data,
                                                const uint8_t *rx,
                                                size_t size) {
  if (!app || !data || !rx || size == 0U)
    return AzV3CommandNotFound;

  if (app->v3_sector_select_pending) {
    app->v3_sector_select_pending = false;
    if (size != 4U || rx[0] == 0xFFU)
      return AzV3CommandNotProcessedNak;
    app->v3_sector = rx[0];
  ...
  case AZ_NFC_CMD_READ: {
    ...
    if (!az_v3_page_valid(app, start_page) ||
  ...
  case AZ_NFC_CMD_FAST_READ: {
    ...
    if (end_page < start_page ||
        !az_v3_range_valid(app, start_page, end_page) ||
        !az_v3_check_access(app, data, start_page, false) ||
  ...
  case AZ_NFC_CMD_GET_VERSION:
    if (size != 1U)
      return AzV3CommandNotFound;
    az_v3_send_standard(app, (const uint8_t *)&data->version,
                        sizeof(data->version));
    return AzV3CommandProcessed;
  ...
  case AZ_NFC_CMD_READ_SIG:
    if (size != 2U)
      return AzV3CommandNotFound;
    az_v3_send_standard(app, data->signature.data,
                        sizeof(data->signature.data));
    return AzV3CommandProcessed;
  ...
  case AZ_NFC_CMD_PWD_AUTH: {
    ...
    MfUltralightConfigPages *config =
        (MfUltralightConfigPages *)&data->page[AZ_V3_CONFIG_PAGE];
    const bool success = memcmp(config->password.data, rx + 1U, 4U) == 0;
    if (az_v3_auth_locked(data, config, success))
      return AzV3CommandNotProcessedAuthNak;
    if (!success)
      return AzV3CommandNotProcessedNak;
...
/**
 * @brief Convert a version-3 command result into listener control flow and
 * output.
 * @param app Application state.
 * @param result Internal version-3 command result.
 * @return The computed result value.
 */
static NfcCommand az_v3_postprocess(AmiiboZeroApp *app,
                                    AzV3CommandResult result) {
  if (result == AzV3CommandProcessedAck) {
    az_v3_send_short(app, AZ_NFC_ACK);
    return NfcCommandContinue;
  }
  if (result == AzV3CommandProcessedSilent)
    return NfcCommandReset;
  if (result == AzV3CommandProcessed)
    return NfcCommandContinue;
 ...
 * @return The computed result value.
 */
static NfcCommand az_v3_listener_callback(NfcGenericEvent event,
                                          void *context) {
  AmiiboZeroApp *app = context;
  if (!app || event.protocol != NfcProtocolIso14443_3a || !event.event_data)
  ...
  if (iso_event->type == Iso14443_3aListenerEventTypeReceivedStandardFrame) {
    if (!iso_event->data || !iso_event->data->buffer)
      return NfcCommandSleep;
    ...
    return az_v3_postprocess(
        app, az_v3_dispatch_command(app, data, bit_buffer_get_data(buffer),
                                    payload_size));
  }
...
bool az_nfc_listener_start(AmiiboZeroApp *app) {
  if (!app || app->emulating || app->listener || app->v3_i2c_listener)
    return false;
  ...
  if (!is_v3) {
    app->listener =
        nfc_listener_alloc(app->nfc, NfcProtocolMfUltralight, device_data);
    if (!app->listener)
      return false;
  ...
  app->listener = nfc_listener_alloc(app->nfc, NfcProtocolIso14443_3a,
                                     data->iso14443_3a_data);
  if (!app->listener)
    return false;
...
/**
 * @brief Stop active emulation and synchronize mutable tag state back to the
 * device.
 * @param app Application state.
 * @return true on success; false if the operation cannot be completed.
 */
bool az_nfc_listener_pause_and_sync(AmiiboZeroApp *app) {
  if (!app || !app->emulating || !app->listener)
    return false;
  ...
  nfc_listener_stop(app->listener);
  const NfcDeviceData *latest =
      nfc_listener_get_data(app->listener, NfcProtocolMfUltralight);
  if (latest)
    nfc_device_set_data(app->nfc_device, NfcProtocolMfUltralight, latest);
  nfc_listener_free(app->listener);
  app->listener = NULL;
  app->emulating = false;
  return latest != NULL;
...
/**
 * @brief Replace the UID of the current Amiibo image while preserving encrypted
 * content.
 * @param device NFC device to inspect or modify.
 * @param keys Loaded Amiibo key material.
 * @return true on success; false if the operation cannot be completed.
...
bool az_nfc_randomize_uid(NfcDevice *device, const AzKeys *keys) {
  if (!device || !keys || !keys->valid || !az_nfc_validate_amiibo(device))
    return false;
  MfUltralightData *data =
  ...
  if (!az_nfc_extract_standard_dump(data, old_dump))
    return false;

  if (data->type == MfUltralightTypeNTAG215) {
    uint8_t new_raw_uid[9];
    if (!az_rekey_dump_uid(old_dump, keys, new_dump, new_raw_uid))
      return false;
  ...
  uint8_t new_uid7[7];
  if (!az_rekey_v3_dump_uid(old_dump, keys, new_dump, new_uid7))
    return false;
  uint8_t *bytes = (uint8_t *)data->page;
  memcpy(bytes, new_dump, AZ_V3_SHIFT_START);
  memcpy(bytes + AZ_V3_SHIFT_START + AZ_V3_SHIFT_SIZE,
         new_dump + AZ_V3_SHIFT_START, AZ_DUMP_SIZE - AZ_V3_SHIFT_START);
 ...
 */
static void az_utf16be_to_utf8(const uint8_t *source, size_t code_units,
  ...
  for (size_t i = 0; i < code_units; i++) {
    uint16_t cp =
        (uint16_t)(((uint16_t)source[i * 2] << 8) | source[i * 2 + 1]);
    if (cp == 0 || cp == 0xFFFFU)
      break;
    if (cp < 0x80U) {
      if (p + 1 >= out_size)
 ...
 * @param out_size Destination for the resulting size.
 */
static void az_format_amiibo_date(const uint8_t bytes[2], char *out,
                                  size_t out_size) {
 ...
 * @return true on success; false if the operation cannot be completed.
 */
bool az_nfc_read_details(const NfcDevice *device, const AzKeys *keys,
                         AzAmiiboDetails *out) {
  if (!out)
    return false;
  ...
  if (!az_nfc_extract_standard_dump(data, encrypted) ||
      !az_decrypt_dump(encrypted, keys, plain)) {
    return false;
  }
  ...
  if (out->initialized) {
    ...
    az_utf16be_to_utf8(plain + 0xBA, 10, out->owner_mii,
                       sizeof(out->owner_mii));
  }
...
bool az_nfc_validate_amiibo(const NfcDevice *device) {
  if (!device || nfc_device_get_protocol(device) != NfcProtocolMfUltralight)
    return false;
  const MfUltralightData *data = (const MfUltralightData *)nfc_device_get_data(
      device, NfcProtocolMfUltralight);
  if (!data)
    return false;
  if (data->type == MfUltralightTypeNTAG215)
    return data->pages_total >= 133;
  if (data->type == MfUltralightTypeNTAGI2CPlus2K)
    return data->pages_total >= AZ_V3_PAGES;
  return false;
}
...
bool az_nfc_extract_figure_id(const NfcDevice *device, uint8_t out_id[8]) {
  if (!out_id || !az_nfc_validate_amiibo(device))
    return false;
  const MfUltralightData *data = (const MfUltralightData *)nfc_device_get_data(
      device, NfcProtocolMfUltralight);
  const uint8_t *bytes = (const uint8_t *)data->page;
...
bool az_nfc_load_device(NfcDevice *device, const char *path) {
  if (!device || !path || !nfc_device_load(device, path))
    return false;
