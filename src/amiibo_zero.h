 ...
 * @details Defines application constants, shared data models, runtime state, and cross-module interfaces.
 */
...
/**
 * @brief Copy a string into a fixed-size destination with guaranteed NUL termination.
...
/** @brief Amiibo category metadata and the number of indexed figures it contains. */
typedef struct {
...
typedef struct {
    ...
    bool valid; /**< Whether the saved record was successfully recognized as an Amiibo. */
} AzSavedEntry;

/** @brief Catalog entry for a Lock-On SRAM payload. */
typedef struct {
    char filename[96]; /**< Filename of the Lock-On payload. */
    char display_name[AZ_NAME_MAX]; /**< Display label derived from the payload filename. */
    uint8_t source_size; /**< Original payload size encoded by the Lock-On file. */
} AzLockOnEntry;
...
typedef struct {
    ...
    bool initialized; /**< Whether the Amiibo owner/profile area is initialized. */
    bool app_data_initialized; /**< Whether the application-specific data area is initialized. */
    ...
    uint64_t application_id; /**< Application identifier stored by the game data area. */
...
/** @brief High-level phases reported while validating or rebuilding the database index. */
typedef enum {
...
struct AmiiboZeroApp {
    ...
    UiManager* ui; /**< C++ UI manager that owns views and the screen stack. */
    ...
    bool current_is_saved; /**< Whether the current figure is backed by a saved NFC file. */
    char current_saved_filename[96]; /**< Filename backing the current saved figure. */
    AzAmiiboDetails current_details; /**< Decoded details for the current saved figure. */

    uint8_t current_lockon_sram[AZ_LOCKON_SRAM_SIZE]; /**< Lock-On SRAM payload selected for the current figure. */
    bool current_lockon_valid; /**< Whether a current Lock-On SRAM payload is loaded. */
    ...
    FuriThread* db_thread; /**< Background thread used for database preparation. */
    volatile bool db_thread_done; /**< Whether the database worker has finished. */
    bool db_thread_result; /**< Success result returned by the database worker. */
    uint32_t db_thread_count; /**< Figure count produced by the database worker. */
    bool db_thread_force; /**< Whether the worker should force an index rebuild. */
    volatile uint8_t db_progress; /**< Latest database preparation percentage. */
    volatile AzDbProgressStage db_progress_stage; /**< Latest database preparation phase. */

    Nfc* nfc; /**< NFC service instance used for emulation. */
    NfcDevice* nfc_device; /**< Mutable NFC device image for the current Amiibo. */
    NfcListener* listener; /**< Active NFC listener while emulating. */
    BitBuffer* v3_tx_buffer; /**< Reusable transmit buffer for version-3 listener responses. */
    bool v3_i2c_listener; /**< Whether version-3 listener state is operating in the I2C-style memory mode. */
    bool v3_sector_select_pending; /**< Whether a version-3 sector-select command awaits its second frame. */
    bool v3_authenticated; /**< Whether the current version-3 interaction has authenticated successfully. */
    bool v3_sram_ready; /**< Whether version-3 SRAM content is ready for access. */
    uint8_t v3_sector; /**< Currently selected version-3 logical sector. */
    bool emulating; /**< Whether NFC emulation is currently active. */
    bool emulation_persistent; /**< Whether mutable emulation state should be saved on stop. */
    char emulation_path[AZ_PATH_MAX]; /**< Backing file path for persistent emulation. */
};
